...
1. **최근 기록 조회**: `select id, title, dept from public.posts order by id desc limit 5;`
2. **관리자 활동 우선 보고**: 조회 결과에 감사실장(`dept='감사실장'`) 보고가 있으면 **반드시 먼저 강조** — "그 사이 관리자가 입장해 ○○을 했습니다" 형식으로 요약.
3. **이어서 한 줄씩**: 직전 작업 요지 / 미완료·대기 작업 / 세팅 모드 상태(세팅 중이면 관리자 접속 가능함을 명시).
4. 브리핑은 **5줄 내외로 간결하게** — 대통령의 본론을 지연시키지 않고, 브리핑 후 곧바로 대통령의 용건에 응한다.
