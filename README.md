...
- `.github/workflows/auto-publish.yml` — 작업 브랜치(`claude/**`) push 시 자동으로 `main`에 머지 + Pages 배포
- `CLAUDE.md` — 조직 체계 정의 & 대화 기록 규칙

### 자동 공개 흐름

작업 브랜치(`claude/**`)에 커밋이 push되면 `auto-publish.yml`이 자동으로 `main`에 머지하고 GitHub Pages에 배포합니다. 즉 **"대화끝" 기록이 별도 조작 없이 웹사이트에 자동 공개**됩니다.
